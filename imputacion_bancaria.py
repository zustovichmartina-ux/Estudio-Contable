...
def df_extracto_a_movs_imputacion(df) -> list[dict[str, Any]]:
    """Convierte el DataFrame unificado del estudio al dict del demo."""
    if df is None or getattr(df, "empty", True):
        return []
    out: list[dict[str, Any]] = []
    for _, row in df.iterrows():
        if str(row.get("Tipo fila") or "Movimiento") == "Saldo inicial":
            continue
        credito = float(row.get("Credito") or 0 or 0)
        debito = float(row.get("Debito") or 0 or 0)
        try:
            importe = float(row.get("Importe") or 0)
        except (TypeError, ValueError):
            importe = credito - debito
        if credito <= 0 and debito <= 0 and importe:
            if importe > 0:
                credito = importe
            else:
                debito = abs(importe)
        saldo = row.get("Saldo")
        try:
            saldo_f = float(saldo) if saldo is not None and str(saldo) not in {"", "None", "nan"} else None
        except (TypeError, ValueError):
            saldo_f = None
        fecha = row.get("Fecha")
        if hasattr(fecha, "strftime"):
            ...
            fecha_txt = str(fecha or "")
        out.append(
            {
                "fecha": fecha_txt,
                "descripcion": str(row.get("Descripcion") or ""),
                "detalle": str(row.get("Detalle") or "") if row.get("Detalle") is not None else "",
                "credito": round(credito, 2),
                "debito": round(debito, 2),
                "monto": round(credito - debito, 2),
                "saldo": round(saldo_f, 2) if saldo_f is not None else None,
